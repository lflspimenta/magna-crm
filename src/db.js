...
const fromDB = (row, mapping) => { if (!row) return null; const out = { ...row }; for (const [appKey, dbKey] of Object.entries(mapping || {})) { if (dbKey in out) { out[appKey] = out[dbKey]; if (appKey !== dbKey) delete out[dbKey]; } } return out; };
...
function makeCRUD(table, mapping, allowed, opts = {}) {
  ...
  return {
    async list() { if (!supa) return []; const { data, error } = await supa.from(table).select('*').order(orderBy, { ascending }); if (error) return []; return (data || []).map(r => fromDB(r, mapping)); },
...
export async function uploadFoto(file) { const name = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${file.name.split('.').pop()}`; const { error } = await supa.storage.from('fotos-imoveis').upload(name, file); if (error) throw new Error(error.message); return supa.storage.from('fotos-imoveis').getPublicUrl(name).data.publicUrl; }
...
export async function uploadDocumento(file, pid) { const name = `${pid}/${Date.now()}-${file.name}`; const { error } = await supa.storage.from('documentos-proprietarios').upload(name, file); if (error) throw new Error(error.message); return supa.storage.from('documentos-proprietarios').getPublicUrl(name).data.publicUrl; }
